...
//                .fetchCount();
    ...
    void concat() {
        List<String> result = queryFactory
                .select(member.username.concat("_").concat(member.age.stringValue()))   //int타입을 문자열타입으로 바꿔줌
                .from(member)
                .where(member.username.eq("member1"))
                .fetch();
    ...
    //queryDsl필드버전 (getter, setter없어도됨)
    @Test
    void findDtoByField() {
    ...
    //quertyDsl 생성자버전(엔티티와 DTO의 필드타입이 일치해야됨)
    @Test
    ...
    @Test
    void sqlFunction2() {
        //ANSI표준함수는 queryDsl이 기본적으로 제공한다
        List<String> result = queryFactory
                .select(member.username)
                .from(member)
